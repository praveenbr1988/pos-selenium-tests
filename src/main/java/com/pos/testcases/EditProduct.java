...
public class EditProduct extends ProjectSpecificMethods {
	...
	public void runLogin(String productId, String changeProductName, String verifyPopup) {
		new LoginPage().login().clickProductMenu().enterId(productId).clickProductEditIcon()
				.changeProductName(changeProductName).clickSubmitButton().verifyEditedProductPopup(verifyPopup)
				.clickOKButton().enterId(productId).verifyProductNameChanged(productId, changeProductName);
